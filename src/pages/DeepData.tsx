  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {/* Industry Chain */}
        <Card className="col-span-1 md:col-span-2 p-8 border-none bg-gradient-to-br from-orange-500 to-amber-400 text-white overflow-hidden relative group">
          <div className="absolute -right-20 -bottom-20 w-80 h-80 bg-orange-500/20 rounded-full blur-3xl group-hover:bg-orange-500/30 transition-all"></div>
          <div className="relative z-10 h-full flex flex-col">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-auto">
              {['上游供应', '中游制造', '下游分销', '竞争情报'].map(tag => (
                <div key={tag} className="px-4 py-3 bg-card/10 rounded-xl backdrop-blur-sm border border-white/5 text-center text-sm font-medium hover:bg-card/20 transition-colors cursor-pointer">
                  {tag}
                </div>
              ))}
            </div>
            <Button className="w-fit mt-8 bg-orange-500 hover:bg-orange-500 border-none px-8 rounded-xl font-bold">
              开始产业探索
            </Button>
          </div>
        </Card>
        ...
        {/* Global Data */}
        <Card className="p-8 border-none bg-card shadow-xl hover:shadow-2xl transition-all">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-xs p-3 bg-slate-50 rounded-lg">
              ...
              <span className="font-bold text-orange-500">8,500万+</span>
            </div>
            <div className="flex justify-between items-center text-xs p-3 bg-slate-50 rounded-lg">
              <span className="text-slate-600">东南亚/一带一路</span>
              <span className="font-bold text-orange-500">3,200万+</span>
            </div>
          </div>
        ...
        <div className="md:col-span-2 lg:col-span-3">
          <div className="flex items-center justify-between mb-8 mt-4">
            ...
            <Button variant="ghost" className="text-orange-500 font-bold">查看全部报告</Button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <Card key={idx} className="p-6 hover:shadow-lg transition-all group border-slate-100">
                <Badge className="mb-4 bg-slate-100 text-slate-500 border-none group-hover:bg-orange-500/10 group-hover:text-orange-500 transition-colors">
                  {report.category}
                </Badge>
                <h4 className="font-bold text-slate-900 mb-4 line-clamp-2 min-h-[3rem]">{report.title}</h4>
                <div className="flex items-center justify-between text-[10px] text-slate-600 mb-6">
                  <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> 最新发布</span>
                  <span>{report.date}</span>
                </div>
                <Button variant="outline" size="sm" className="w-full gap-2 border-slate-200 hover:border-orange-500 hover:text-orange-500 rounded-lg">
        ...
        {/* Data API Section */}
        <Card className="md:col-span-2 lg:col-span-3 p-8 border-none bg-orange-500/10 flex flex-col md:flex-row items-center justify-between gap-8 mt-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <Layers className="w-5 h-5 text-orange-500" />
              <span className="text-xs font-bold text-orange-500 uppercase tracking-widest">Enterprise API</span>
            </div>
          ...
          <div className="flex gap-4">
            <Button size="lg" className="bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl px-10">获取 API 文档</Button>
            <Button size="lg" variant="outline" className="border-orange-500 text-orange-500 font-bold rounded-xl px-10">联系技术支持</Button>
          </div>
        </Card>
      </div>
    </div>
